 ...
 *
 * 验证：初始牌组名回退、真实牌组加载、showDeckSelector 派生、选真实牌组持久化、切自定义、onPickDeck 回调。
 * mock Settings（deckSelection 依赖）与 AnkiDroid.getDecks。
 */
...
describe('useDeckSelector', () => {
  ...
  it('deckName 不在真实列表时 showDeckSelector 为 false（回退自由输入）', async () => {
